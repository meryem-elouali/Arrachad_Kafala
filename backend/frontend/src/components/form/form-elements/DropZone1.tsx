...
const DropzoneComponent1: React.FC<DropzoneProps> = ({ label, id, onFileSelect }) => {
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      ...
      // si plusieurs fichiers à la fois, tu peux gérer ça différemment
      ...
      if (file.type.startsWith("image/")) {
        ...
        setPreviewUrl(null); // pas de preview pour les fichiers non-images
      }

      onFileSelect(file); // transmettre au parent
    },
    [onFileSelect]
  );
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    multiple: false, // tu peux mettre true si tu veux autoriser plusieurs fichiers
    accept: {
      "image/*": [],
      "application/pdf": [],
      "application/msword": [],
        ...
        >
          ...
          <div className="dz-message flex flex-col items-center text-center">
            ...
            <h4 className="mb-3 font-semibold text-gray-800 text-lg dark:text-white">
              {isDragActive ? "Déposez le fichier ici" : "Glissez un fichier ou cliquez"}
            </h4>
            <span className="text-sm text-gray-600 dark:text-gray-400">
              Images, PDF, Word, Excel acceptés
            </span>
        ...
        <div className="flex flex-col items-center">
          {previewUrl && (
            <img
              src={previewUrl}
              alt="Aperçu"
              className="max-h-[250px] rounded-xl shadow-md object-contain mb-4"
            />
          )}
          <button
